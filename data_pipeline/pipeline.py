...
from .preprocessing import (
    extract_tesseract_information,
    get_groundTruth,
    get_final_groundtruth,
)
...
def get_one_hot_encoded_chargrid(image_file_name: str) -> Union[int, Dict[str, Any]]:
    ...
    # save first output
    np.save(
        os.path.join(outdir_np_chargrid, image_file_name).replace("jpg", "npy"),
        chargrid_np,
    )
    ...
    if np.shape(chargrid_np) != (0, 0):
        ...
        np.save(
            os.path.join(outdir_np_chargrid_reduced, image_file_name).replace(
                "jpg", "npy"
            ),
            chargrid_np,
        )
        np.save(
            os.path.join(outdir_np_gt_reduced, image_file_name).replace("jpg", "npy"),
            gt_np,
        )
        gt_pd.to_pickle(
            os.path.join(outdir_pd_bbox_reduced, image_file_name).replace("jpg", "pkl")
        )
    ...
    chargrid_np_one_hot, gt_np_one_hot = resize_to_target(
        chargrid_np_one_hot, gt_np_one_hot
    )
    ...
    # save last output
    np.save(
        os.path.join(outdir_np_chargrid_1h, image_file_name).replace("jpg", "npy"),
        chargrid_np_one_hot,
    )
    np.save(
        os.path.join(outdir_np_gt_1h, image_file_name).replace("jpg", "npy"),
        gt_np_one_hot,
    )
    np.save(
        os.path.join(outdir_np_bbox_anchor_coord, image_file_name).replace(
            "jpg", "npy"
        ),
        np_bbox_anchor_coord,
    )
    np.save(
        os.path.join(outdir_np_bbox_anchor_mask, image_file_name).replace("jpg", "npy"),
        np_bbox_anchor_mask,
    )
...
def get_one_hot_encoded_chargrid_for_list(
    image_list: list, out_queue: Queue, show_progress: bool = False
):
    processed = []
    if show_progress:
...
def process_dataset(
    dataset_dir_path: str, num_workers: int = 0, save_np_file: bool = False
) -> list:
    if num_workers == 0:
    ...
    list_filenames = [
        ...
        if os.path.isfile(os.path.join(dataset_dir_path, f))
    ]
    ...
    for i in range(num_workers):
        ...
        tmp_list = (
            list_filenames[start_index:]
            if end_index > dataset_length
            else list_filenames[start_index:end_index]
        )

        if len(tmp_list) > 0:
            process = multiprocessing.Process(
                target=get_one_hot_encoded_chargrid_for_list,
                args=(
                    tmp_list,
                    processed_dataset,
                ),
    ...
    print(
        "total time taken for file parsing: {}".format(
            (datetime.now() - time_then).total_seconds()
        )
    )
    return result
